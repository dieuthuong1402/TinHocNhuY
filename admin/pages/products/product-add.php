...
if (!empty($adminlogged)) {
    ...
    if (isset($_POST['add-product-submit']) && $_POST['add-product-submit'] == 'submit') {
        ...
        if(count(explode('||', $hinh_anh)) < 4) {
            $error['hinhanh'] = 'Vui lòng chọn tối thiểu 4 hình ảnh!';
        }


        if(empty($error)) {
            $stmt_insert_product = $conn->prepare("INSERT INTO `san_pham`(`ten_sp`, `don_gia_nhap`, `don_gia_ban`, `so_luong_kho`, `mo_ta`, `hinh_anh`, `ngay_nhap`, `id_loai_hang`) VALUES (? ,? ,? ,? ,? ,? ,? ,? )");
            $stmt_insert_product->bind_param("ssssssss", $tensanpham, $dongianhap, $dongiaban, $soluong, $motasanpham, $hinh_anh, $ngaynhap, $id_loai);
                                    ...
                                    <div class="card mb-4 shadow-none">
                                        <div class="card-body">
                                            <div>
                                                ...
                                                <input type="number" class="form-control" id="dongiaban" name="dongiaban" min="1" aria-describedby="defaultFormControlHelp" />
                                                <span id="defaultFormControlHelp" class="form-text text-danger">
