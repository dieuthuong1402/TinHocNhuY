...
if (!empty($id)) {
    // Sản phẩm
    $stmt_get_product = $conn->prepare("SELECT san_pham.id, ten_sp, don_gia_ban, mo_ta, hinh_anh, loai_hang.ten_loai FROM san_pham, loai_hang WHERE san_pham.id_loai_hang = loai_hang.id AND san_pham.id = ?");
    ...
    // Sản phẩm liên quan
    $stmt_get_related_products = $conn->prepare("SELECT id, ten_sp, don_gia_ban, hinh_anh FROM san_pham WHERE san_pham.id_loai_hang = ? AND id NOT IN (?) ");
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="product-detail__info">
                                ...
                                <p class="product-detail__old-price"><span><?= currency_format($product['don_gia_ban']) ?></span> <span>-10%</span></p>
                                <h6 class="product-detail__new-price"><?= currency_format($product['don_gia_ban'] - $product['don_gia_ban'] * 0.1) ?></h6>

                                <div class="product-detail__btns">
                                    <button type="button" class="btn btn-primary" data-id="<?= $product['id'] ?>">
                                        MUA NGAY
                                    </button>
                                    <button type="button" class="btn btn-outline-primary btn-add-cart" data-id="<?= $product['id'] ?>">
                                        THÊM VÀO GIỎ HÀNG
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="related-product">
                        ...
                        <div class="row related-product__slider">
                            <?php foreach ($related_products as $item) {
                                $thumbnails = explode("||", $item['hinh_anh']) ?>
                                <div class="col-md-3">
            ...
            $('.btn-add-cart').click(function(e) {
                ...
                $.ajax({
                    ...
                    if (response.redirect) {
                        ...
                        //
                        const listCartProducts = response.cart_products;
                        let tongsanpham = 0;
                        let tongtien = 0;
                        ...
                        document.querySelector('.price-total-cart').innerText = `${VND.format(tongtien)}`;
                        document.querySelector('.header-cart-menusub__list').innerHTML = html.join('');
                    }
